...
def idx_vim(bp_series_multiple, individual):
    ...
    x, intercept, r_value, p_value, std_err = linregress(log_sd, log_mean)
...
def idx_time_rate(bp_series, time_series):
    ...
    sum = 0
    for i in range(n - 1):
        sum += np.abs(bp_series[i + 1] - bp_series[i]) / (time_series[i + 1] - time_series[i])
    return sum / n
